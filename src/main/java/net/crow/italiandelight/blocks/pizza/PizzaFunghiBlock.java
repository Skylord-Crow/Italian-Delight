...
public class PizzaFunghiBlock extends PizzaBlock {
    public PizzaFunghiBlock(BlockBehaviour.Properties properties) {
        super(properties);
    ...
    private InteractionResult takeServing(Level level, BlockPos pos, BlockState state, Player player, InteractionHand handIn) {
        ...
        if (!heldStack.isEmpty() && !heldStack.getItem().equals(ItemInit.PIZZA_FUNGHI_SLICE.get())) {
            popResource(level, pos, new ItemStack((ItemLike)ItemInit.PIZZA_FUNGHI_SLICE.get(), 1));
        } else if (heldStack.getItem().equals(ItemInit.PIZZA_FUNGHI_SLICE.get()) && heldStack.getCount() < heldStack.getMaxStackSize()) {
            heldStack.setCount(heldStack.getCount() + 1);
        } else {
            player.setItemInHand(handIn, new ItemStack((ItemLike)ItemInit.PIZZA_FUNGHI_SLICE.get()));
        }
